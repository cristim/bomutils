...
void write_bom( istream & lsbom_file, const string & output_path, bool simplified = false ) {
  Node root;
  unsigned int num;
  root.type = kRootNode;
  {
    ...
    while( getline( lsbom_file, line ) ) {
      ...
      if (simplified) {
        // Apple's -s -i input contains paths alone, as emitted by lsbom -s.
        // The root record is implicit in the BOM tree.
        if (line == "." || line == "./") continue;
        if (line.compare(0, 2, "./") == 0) line.erase(0, 2);
        if (line.empty() || line[0] == '/' || line.back() == '/' ||
            line.find("//") != string::npos || line == ".." ||
            line.compare(0, 3, "../") == 0 || line.find("/../") != string::npos ||
            (line.size() >= 3 && line.compare(line.size() - 3, 3, "/..") == 0)) {
          cerr << "Invalid path in simplified BOM input: " << line << endl;
          exit(1);
        }
        name = line;
        n.type = kFileNode;
    ...
    if (simplified) {
      // Directory records are needed to form the BOM tree even though -s
      // carries no file metadata. Every parent must be listed explicitly.
      for (const auto& entry : all_nodes) {
        size_t slash = entry.first.find('/');
        while (slash != string::npos) {
          auto parent = all_nodes.find(entry.first.substr(0, slash));
          if (parent == all_nodes.end()) {
            cerr << "Parent directory of \"" << entry.first << "\" does not appear in list" << endl;
            exit(1);
          }
          parent->second.type = kDirectoryNode;
          parent->second.mode = 0040755;
          slash = entry.first.find('/', slash + 1);
        }
      }
    }
    /* create tree */
...
void usage() {
  cout << "Usage: mkbom [-s] [-i] [-u uid] [-g gid] source target-bom-file" << endl << endl;
  cout << "\t-s\tCreate a simplified BOM from paths alone" << endl;
...
int main( int argc, char * argv[] ) {
  ...
  while (true) {
    signed char c = getopt(argc, argv, "hisu:g:");
    if (c == -1) {
      break;
    }
  ...
  if ( isFileListSource ) {
    ...
    if (simplified) {
      stringstream rich_list(buffer);
      stringstream paths;
      string line;
      while (getline(rich_list, line)) {
